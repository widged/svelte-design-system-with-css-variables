<script>
  import CheckableCard from '../CheckableCard.svelte';
  const discord_i = `<svg xmlns="http://www.w3.org/2000/svg" width="192" height="192" fill="currentColor" viewBox="0 0 256 256">
...
<div class="px-8">
  <h1>Scenario -- default</h1>
  <CheckableCard label="Discord" icon={discord_i} />
  <h1>Scenario -- selected</h1>
  <CheckableCard label="Discord" icon={discord_i} checked={true} />
  <h1>Scenario -- disabled</h1>
  <CheckableCard label="Discord" icon={discord_i} disabled={true} />
</div>
...
<style>
  ...
  :root {
    --grey-600: #555d6e;
    --color-prose: var(--grey-600);
    --color-checked: #4299e1;
